...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for lock in locks:
        ...
        _LOGGER.info(f"KevoPlus: Attempting to fetch lock {lock_id}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    _LOGGER.error(f"KevoPlus: Error fetching lock {lock_id}")
                    raise
                else:
        ...
        _LOGGER.info(f"KevoPlus: Fetched lock {lock_id} ({kevo.name})")
    ...
    def lock(self, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    _LOGGER.error(f"KevoPlus: Error locking lock {self._name}")
                    raise
                else:
                    time.sleep(retry_delay)
            else:
    ...
    def unlock(self, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    _LOGGER.error(f"KevoPlus: Error unlocking lock {self._name}")
                    raise
                else:
                    time.sleep(retry_delay)
            else:
